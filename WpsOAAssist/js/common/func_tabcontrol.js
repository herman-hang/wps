...
function OnBtnSaveToServer() {
	...
	var publicationId = GetDocParamsValue(l_doc, "publicationId"); // 刊物ID
    ...
    if (pIsOnlineOADoc(l_doc) == false) {
        ...
        try {
            //调用OA助手的上传方法
            UploadFile(l_UploadName, l_DocPath, l_uploadPath, l_FieldName, OnUploadToServerSuccess, OnUploadToServerFail, token, publicationId);
        } catch (err) {
